...
function TASK.new(code,...)
    ...
    local targetProfiler = hook and select(2, debug.getupvalue(hook, 4))
    if targetProfiler then
        targetProfiler.profiledTasks[code] = debug.getinfo(2, "Sl")
    end
    local thread=coroutine.create(code)
    assert(resume(thread,...))
    if targetProfiler then
        targetProfiler.profiledTaskCoroutines[thread] = true
    end
