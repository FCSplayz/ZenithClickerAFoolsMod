...
log{COLOR.lY,"BGM reset, SFX command, multi-command delimiter, and inline Lua syntax by FCSplayz."}
    ...
    commands.sfx={
        code=function(arg)
            ...
            if #arg>=2 and not tonumber(arg[2]) then
                log{COLOR.R,("sfxVolume is not a number")}
            elseif #arg>=3 and not tonumber(arg[3]) then
                log{COLOR.R,("sfxPosition is not a number")}
            elseif #arg>=4 and not tonumber(arg[4]) then
                log{COLOR.R,("sfxPitch is not a number")}
            elseif #arg>=1 and arg[1]~='' then
                SFX.play(arg[1], arg[2]~='' and tonumber(arg[2]) or 1, arg[3]~='' and arg[3] or 0, arg[4]~='' and arg[4] or 0)
            else
                log{COLOR.I,"Usage: sfx [sfxName] (sfxVolume) (sfxPosition) (sfxPitch)"}
            end
        end,
        ...
        details={
            ...
            "sfx",
            "sfx [bgmName]",
            "sfx [sfxName] (sfxVolume) (sfxPosition) (sfxPitch)",
        },
    }
    commands.bgm={
        code=function(bgm)
            if bgm=='reset' then
                GAME.updateBgm('init')
                BGM.set('piano', 'volume', GAME.mod.NH == 0 and 1 or GAME.mod.NH == 1 and .26 or 0)
                local v = 2 ^ ((GAME.mod.GV + (GAME.mod.GV == 2 and URM and 1 or 0) - (GAME.mod.PN == 2 and URM and 0.5 or 0)) / 12)
                BGM.set('all', 'pitch', v, .26)
                BGM.set('piano2', 'pitch', 2 * v, .26)
                BGM.set('all', 'highgain', GAME.mod.IN == 0 and 1 or GAME.mod.IN == 1 and .8 or not URM and .626 or .55)
            ...
            elseif bgm~='' then
                ...
                BGM.stop()
                log{COLOR.I,"Usage: bgm <[bgmName]|reset>"}
            end
        end,
...
local function split_outside_braces(str)
    local result = {}
    local buffer = {}
    local depth = 0
    ...
    for i = 1, #str do
        ...
        elseif ch == "}" then
            depth = math.max(depth - 1, 0)  -- clamp depth to 0
            table.insert(buffer, ch)
        elseif ch == "@" and depth == 0 then
            table.insert(result,tostring(table.concat(buffer):gsub("^%s+", ""):gsub("%s+@", "")))
            buffer = {}
        else
            table.insert(buffer, ch)
        end
    ...
    if #buffer > 0 then
        ins(result,tostring(table.concat(buffer):gsub("^%s+", ""):gsub("%s+@", "")))
    end

    return result
end

local function extract_valid_blocks(input)
    local results = {}
    local seen = {}

    for i=1, #input do
        local start = input:find("{#", i, true)
        ...
        while j <= #input and depth > 0 do
            local prev1 = input:sub(j - 1, j - 1)
            local char = input:sub(j, j)
            local next1 = input:sub(j + 1, j + 1)

            if not in_string then
                if char == "\"" or char == "'" then
                    in_string = char
                elseif char == "[" and next1 == "[" then
                    in_string = "]]"
                end

                -- Enforce valid `#` usage
                if char == "#" then
                    local after = input:sub(j + 1, j + 1)
                ...
                if char == "@" then
                    local incr = 1
                    while next1 == " " do
                        next1 = input:sub(j + 1 + incr, j + 1 + incr)
                    end
                    if next1 ~= "#" then
                        valid = false
                        break
                    end
                    next1 = input:sub(j + 1, j + 1)
                end
            ...
            elseif input:sub(j, j + #in_string - 1) == in_string and prev1 ~= "\\" then
                in_string = nil
            end
        ...
        if depth == 0 and valid then
            local block = input:sub(start, j - 1)
...
function scene.keyDown(key,isRep)
    if key=='return' or key=='kpenter' then
        if love.keyboard.isDown('lshift', 'rshift') then
            ...
            local errored = false
            while not errored and #extract_valid_blocks(input) > 0 do
                for _, luaArg in ipairs(extract_valid_blocks(input)) do
                    log{COLOR.LK,luaArg}
                    local luaCmds = STRING.split(string.sub(luaArg, 2, -2), "@")
                    for l = 1, #luaCmds do
                        log{COLOR.lC, "> " .. luaCmds[l]:gsub("^%s+", "")}
                        local code, err = loadstring(luaCmds[l]:gsub("^%s+", ""):gsub("%s+@%s+", "@"):sub(2))
                        if code then
                            local resultColor
                            if sumode then
                            ...
                            if suc then
                                if res~=nil then
                                    ...
                                    input = string.gsub(input, luaArg:gsub("([^%w])", "%%%1"), tostring(res))
                                else
                                    log{resultColor,"done"}
                                    local escaped = luaArg:gsub("([^%w])", "%%%1")  -- escape pattern chars
                                    input = input:gsub(escaped .. "%s*@%s*", ""):gsub(escaped, "")
                                end
                            else
                                errored = true
                                log{COLOR.R, res}
            ...
            cmds = split_outside_braces(input)
            for i=1, #cmds do
                if errored then
                    break
                end
                -- Execute
                if cmds[i]:byte()==35 then
                    -- Execute lua code
                    log{COLOR.lC,"> "..cmds[i]}
                    local code,err=loadstring(cmds[i]:sub(2))
                    if code then
                        local resultColor
                        if sumode then
                            resultColor=COLOR.lY
                    ...
                    local cmd,arg
                    if p then
                        cmd=cmds[i]:sub(1,p-1):lower()
                        arg=cmds[i]:sub(cmds[i]:find("%S",p+1) or -1)
                    else
                        cmd=cmds[i]
                        arg=''
                    end
