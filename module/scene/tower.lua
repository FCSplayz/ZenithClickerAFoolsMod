...
local MD = ModData
ShortCut = {}
for i = 1, 11 do ShortCut[i] = GC.newText(FONT.get(50)) end
...
local function mouseMove(x, y, id)
    SetMouseVisible(true)

    if revHold[id] then return end
...
local function keyPress(key)
    ...
    if bindID and bindID <= 22 and (M.AS > 0 or (not GAME.playing and (bindID == 10 or bindID == 21))) then
        if bindID > 11 then bindID = bindID - 11 end
    ...
    elseif bindID == 24 then
        ...
        GAME.cancelAll()
        if not GAME.achv_talentlessH then GAME.achv_talentlessH = GAME.roundHeight end
    elseif bindID == 25 or bindID == 26 then
        GAME.nixPrompt('keep_no_keyboard')
        scene.mouseDown(MX, MY, bindID == 25 and 1 or 2)
        scene.mouseUp(MX, MY, bindID == 25 and 1 or 2)
        if not GAME.achv_talentlessH then GAME.achv_talentlessH = GAME.roundHeight end
    elseif bindID == 23 then
        GAME.nixPrompt('keep_no_keyboard')
    ...
    elseif key == '`' then
        if GAME.playing then
            ...
            SFX.play('menuhit' .. 3 - math.abs(menuhitNum % 4 - 2))
            menuhitNum = menuhitNum % 4 + 1
            SCN.go('stat', 'none')
        end
        local W = scene.widgetList.stat
        W._pressTime = W._pressTimeMax * 2
        W._hoverTime = W._hoverTimeMaxend
    elseif key == 'backspace' and TEST and GAME.playing then
...
function scene.load()
    ...
    RevUnlocked = TABLE.countAll(GAME.completion, 0) < 10
    TABLE.clear(touchesActive)

    CancelNextClick = true
    CancelNextKeyClick = true

    for i = 1, 11 do ShortCut[i]:set(STAT.keybind[i]:upper()) end
    ...
    ]]):repD(STAT.keybind[23]:upper(), STAT.keybind[24]:upper())
...
function scene.mouseMove(x, y, _, dy, id)
...
local function getBtnPressed()
    ...
    if not (M.NH == 2 and M.AS == 0) then
        if kbIsDown(STAT.keybind[25]) then btnPressed = btnPressed + 1 end
        if kbIsDown(STAT.keybind[26]) then btnPressed = btnPressed + 1 end
    end
    return btnPressed
end
...
function scene.touchMove(x, y, dx, dy, id)
    if (dx == 0 and dy == 0) or revHold[id] then return end
    scene.mouseMove(x, y, dx, dy)
end

function scene.touchDown(x, y, id, _, widgetSel)
    if not usingTouch and MOBILE then
        usingTouch = true
    elseif usingTouch and not MOBILE then
        usingTouch = false
    end
    if usingTouch then
        table.insert(touchesActive, id)
    end
    if GAME.zenithTraveler then return end
    if not widgetSel then
        local x1, y1 = SCR.xOy_dl:inverseTransformPoint(SCR.xOy:transformPoint(x, y))
        local x2, y2 = SCR.xOy_dr:inverseTransformPoint(SCR.xOy:transformPoint(x, y))
        local cond = false
        if STAT.revControlType == 1 then
            cond = x1 <= 200
        elseif STAT.revControlType == 2 then
            cond = x2 >= -200
        elseif STAT.revControlType == 3 then
            cond = x1 <= 200 or x2 >= -200
        end
        local posAdjust = -40 * (1 - GAME.uiHide) + DeckPress
        if (not GAME.playing or M.PN > 0) and cond and MATH.between(y1, -560 + 6 * GAME.uiHide + posAdjust, posAdjust) then
            revHold[id] = true
            return
        else scene.mouseMove(x, y, 0, 0, id) end
    end
...
function scene.touchUp(x, y, id)
    if usingTouch then
        for i = #touchesActive, 1, -1 do
            if touchesActive[i] == id then
                table.remove(touchesActive, i)
                break
            end
        end
    end
...
FoolsDayTextColor = { 0, 1, .6 }
FoolsDayShadeColor = { .1, .5, .3 }
BaseTextColor = { .7, .5, .3 }
...
function COLOR.lerp(colorA, colorB, t)
    assert(type(colorA) == 'table', "COLOR.lerp(colorA, colorB, t): Need table for colorA")
    assert(type(colorB) == 'table', "COLOR.lerp(colorA, colorB, t): Need table for colorB")
    assert(type(t) == 'number', "COLOR.lerp(colorA, colorB, t): Need number for t")
    return
        colorA[1] * (1 - t) + colorB[1] * t,
        colorA[2] * (1 - t) + colorB[2] * t,
        colorA[3] * (1 - t) + colorB[3] * t,
        colorA[4] * (1 - t) + colorB[4] * t
end
...
function scene.draw()
    ...
    local panelH = 697 + GAME.uiHide * (420 + GAME.height / 6.2) + (DeckPress < 0 and DeckPress or 0) / 2
    ...
    gc_push('transform')
    gc_scale(1, 1 - ((DeckPress < 0 and DeckPress or 0) / 606))
    gc_translate(0, DeckPress * GAME.uiHide)
    ...
    if not GAME.playing then
        ...
        gc_setColor(GAME.dailyState == 2 and DailyActived and COLOR.lR or TextColor)
        gc_mDraw(TEXTS.dcBest, -200, 100, nil, .626)
        gc_mDraw(TEXTS.dcTimer, -200, 152, nil, .626)
        if DailyActived then
            if GAME.dailyState == 2 then
                gc_setColor(COLOR.R)
            end
...
function scene.overDraw()
    ...
    if GAME.playing then
        ...
        for i = 1, #GAME.quests do
            ...
            local text = Q.name
            local kx = min(Q.k, 1550 / text:getWidth()) * (Q.flipX and -1 or 1)
            local ky = max(kx, Q.k) * (Q.flipY and -1 or 1)
            local a = 1
    ...
    if usingTouch and (not GAME.playing or M.PN > 0 or GAME.lightHide ~= 1) and RevUnlocked then
        gc_replaceTransform(SCR.xOy_dl)
        if GAME.colorProg == 0 then
            gc_setColor(COLOR.C)
            gc_setAlpha(next(revHold) and .872 or .62)
        elseif GAME.colorProg == 1 then
            gc_setColor(COLOR.S)
            gc_setAlpha(next(revHold) and .42 or .26)
        else
            gc_setColor(COLOR.lerp(COLOR.C, COLOR.S, GAME.colorProg))
            gc_setAlpha(next(revHold) and MATH.lerp(.872, .42, GAME.colorProg) or MATH.lerp(.62, .26, GAME.colorProg))
        end
        local normalPosition = -200 * (GAME.invisDashboard and 1 - GAME.lightHide or GAME.lightHide)
        if STAT.revControlType ~= 2 then
            gc_draw(
                TEXTURE.transition,
                normalPosition,
                -40 * (M.PN == 0 and 1 or (1 - GAME.uiHide)) + DeckPress,
                0,
                200 / 128,
                -560 + (M.PN == 0 and 0 or 6 * GAME.uiHide)
            )
        end
        if STAT.revControlType >= 2 then
    ...
    if GAME.uiHide < 1 then
        ...
        if revT > 0 then
            gc_draw(TEXTS.slogan, 6, 2 + (exT + revT) * 42 - (2 + (exT + revT) * 42 ~= 2 and DeckPress or 0), 0, 1, 1, 0, TEXTS.slogan:getHeight())
            gc_draw(TEXTS.slogan_EX, 6, 2 + (1 - exT + revT) * 42 - (2 + (1 - exT + revT) * 42 ~= 2 and DeckPress or 0), 0, 1, 1, 0, TEXTS.slogan_EX:getHeight())
            gc_draw(TEXTS.slogan_rEX, 6, 2 + (1 - revT + upnT) * 42 - (2 + (1 - revT + upnT) * 42 ~= 2 and DeckPress or 0), 0, 1, 1, 0, TEXTS.slogan_rEX:getHeight())
            gc_draw(TEXTS.slogan_uPN, 6, 2 + (1 - upnT) * 42 - (2 + (1 - upnT) * 42 ~= 2 and DeckPress or 0), 0, 1, 1, 0, TEXTS.slogan_uPN:getHeight())
        else
            gc_draw(TEXTS.slogan, 6, 2 + exT * 42 - (2 + exT * 42 ~= 2 and DeckPress or 0), 0, 1, 1, 0, TEXTS.slogan:getHeight())
            gc_draw(TEXTS.slogan_EX, 6, 2 + (1 - exT) * 42 - (2 + (1 - exT) * 42 ~= 2 and DeckPress or 0), 0, 1, 1, 0, TEXTS.slogan_EX:getHeight())
        end
        gc_replaceTransform(SCR.xOy_dr)
        gc_translate(0, DeckPress)
    ...
    if not GAME.playing and FloatOnCard then
        local C = Cards[FloatOnCard]
        local infoID = C.lock and (C.id == 'DP' and 'lockDP' or C.id == 'PN' and 'lockPN' or C.id == 'SB' and 'lockSB' or 'lock') or C.id
...
scene.widgetList = {
    WIDGET.new {
        name = 'skip', type = 'button',
        pos = { 1, 0 }, x = 90, y = 140, w = 160, h = 60,
        color = { COLOR.HEX "811331" },
        sound_hover = 'menutap',
        fontSize = 30, text = "SKIP   ", textColor = { COLOR.HEX "DE3163" },
        onPress = function()
            local W = scene.widgetList.skip
            W._pressTime = W._pressTimeMax * 2
            W._hoverTime = W._hoverTimeMax
            W._pressed = false
            if GAME.playing then
    ...
    WIDGET.new {
        ...
        onPress = function(key)
            ...
            local ultraKeyed = next(revHold) ~= nil or key == 2 or KBIsDown('lctrl', 'lalt', 'rctrl', 'ralt') or (STAT.keybind[27] ~= "<Unbound>" and KBIsDown(STAT.keybind[27]))
            if ultraKeyed and RevUnlocked then
                ForceURM = not ForceURM
                local W = SCN.scenes.tower.widgetList.help2
                W.frameColor = ForceURM and COLOR.R or TextColor
                W.textColor = ForceURM and COLOR.R or TextColor
                W.floatFrameColor = ForceURM and COLOR.dR or COLOR.DL
                W.floatTextColor = ForceURM and COLOR.R or COLOR.dL
                SFX.play('level500', .3, 0, Tone(ForceURM and 4 or -4))
                SFX.play('level100', .5, 0, Tone(ForceURM and 2 or -2))
                SFX.play('level10', .7, 0, Tone(ForceURM and 1 or -1))
                SFX.play('level1', .9, 0, Tone(ForceURM and .5 or -.5))
                SFX.play('allclear', .10626, 0, Tone(ForceURM and .25 or -.25))
                SFX.play('i', 1, 0, Tone(ForceURM and .5 or -.5))
                SFX.play('t', 1, 0, Tone(ForceURM and .5 or -.5))
                SFX.play('i', .75, 0, Tone(ForceURM and 2.5 or -2.5))
                SFX.play('t', .75, 0, Tone(ForceURM and 2.5 or -2.5))
                SFX.play('z', .75, 0, Tone(ForceURM and 2.5 or -.5))
                if PieceSFXID ~= 6 and PieceSFXID ~= 8 then
                    URM = not URM
                    if URM then
                        TWEEN.new(GAME.anim_setLightColorProg_rev):setDuration(GAME.slowmo and 2.6 or .26):setUnique('colorProg'):run()
                    else
                        TWEEN.new(GAME.anim_setLightColorProg):setDuration(GAME.slowmo and 1.26 or .126):setUnique('colorProg'):run()
                    end
                    if GAME.anyRev then
                        if M.PN == 2 then
                            if ForceURM then
                ...
                PieceSFXID = PieceSFXID % 8 + 1
                URM = (ForceURM and PieceSFXID ~= 8) or (PieceSFXID == 6 and RevUnlocked)
                if PieceSFXID <= 7 then
                    local piece = ('zsjltoi'):sub(PieceSFXID, PieceSFXID)
                    SFX.play(piece, 1, 0, Tone(0))
                    if ForceURM and PieceSFXID == 1 then
                        TWEEN.new(GAME.anim_setLightColorProg_rev):setDuration(GAME.slowmo and 1.26 or .126):setUnique('colorProg'):run()
                        if GAME.anyRev then
                            if M.PN == 2 then
                                TWEEN.new(function(t)
                                    GAME.upnTimer = t
                    ...
                    SFX.play('allclear')
                    if ForceURM then
                        TWEEN.new(GAME.anim_setLightColorProg):setDuration(GAME.slowmo and 2.6 or .26):setUnique('colorProg'):run()
                        if GAME.anyRev then
                            if M.PN == 2 then
                                TWEEN.new(function(t)
                                    GAME.upnTimer = 1 - t
                ...
                if PieceSFXID == 5 then
                    TWEEN.new(GAME.anim_setLightHide):setDuration(GAME.slowmo and 2.6 or .26):setUnique('lightHide'):run()
                elseif PieceSFXID == 6 then
                    if GAME.anyRev then GAME.refreshPBText() end
                    TWEEN.tag_kill('colorProg')
                    if not ForceURM then
                        TWEEN.new(GAME.anim_setLightColorProg_rev):setDuration(GAME.slowmo and 2.6 or .26):setUnique('colorProg'):run()
                    else GAME.colorProg = 0 end
                    TWEEN.new(GAME.anim_setLightHide):setDuration(GAME.slowmo and 1.26 or .126):setUnique('lightHide'):run()
                elseif PieceSFXID == 7 then
                    if GAME.anyRev then GAME.refreshPBText() end
                    if not ForceURM then
                        TWEEN.new(GAME.anim_setLightColorProg):setDuration(GAME.slowmo and 1.26 or .126):setUnique('colorProg'):run()
                    end
                end
                if not ForceURM and M.PN == 2 then
                    if PieceSFXID == 6 then
                        TWEEN.new(function(t)
                            GAME.upnTimer = t
                        end):setUnique('upnSwitched'):setDuration(.26):run()
                    elseif PieceSFXID == 7 then
                ...
                if PieceSFXID == 7 and not GC.isWireframe() then
                    SFX.play('maintenance', .47807, 0, Tone(-4))
                    MSG({
                        cat = 'dark',
        ...
        visibleFunc = function() return not GAME.playing and TABLE.countAll(GAME.completion, 0) < 10 end,
    },
    WIDGET.new {
        name = 'help', type = 'hint',
